...
// NHO: could be wrapped into a function like buildGameBoard to allow you to recall later / i.e. on reset
...
function populateNumbers(column) {
  for (var i = 0; i < values.length; i++) {
    // NHO: this works, but in general would advise against adding attributes that are not known by the browser, i.e. difficulty
    // this can lead to unexpected behavior, if you want to go down that route, prefix this attributes
    // with "data-" so that the browser ignores it then.
    $("#" + column).append("<button class='question' difficulty=" + i + ">" + values[i] + "</button>")
  }
}

// NHO: This structure works, but overall I think it would greatly reduce your code's complexity by
//  restructuring your data. For example `var questions` could be organized as an array of arrays,
// representing each "difficulty", which is full of individual objects for each question.
var questions = {
  col0: {
    diff0: {
...
function goToQuestion() {
  ...
  // NHO: Like that you have this is here for debugging purposes, but once you deploy
  // consider removing all console.logs // commented out code
  console.log(this);
...
$(".question").on("click", function() {
  ...
  var difficulty = $(this).attr("difficulty");
  var category = $(this).parent().attr("id");
  // NHO: while this works, I'm going to consider using eval like this an Anti-pattern that could lead to bugs down the road / confusing future self
  ...
  $("input").on("keypress", function(e) {
    if (e.keyCode == 13) {
      ...
      if (answerText.includes(userInput.toLowerCase()) || userInput.toLowerCase() == answerText) {
        // NHO: score errors out sometimes, might need to correct for typeness  
...
// NHO: you use the same selector here 3 times, might want to save that element to a variable for easy reference
...
// NHO: How could you refactor so that "resetting" does not involve reloading the page?
// Another way to ask that, is what data values / game state needs to be "reset" when the game is over?
$("#reset").on("click", function() {
  location.reload()
})

// NHO: Overall, nice job identifying the problem, breaking it down into steps, and wiring it all together -
// you built a reasonably complex game after only ~3 weeks, congrats!


// Some ideas for future refactoring / consideration:

// Overall I recommend coming up with a certain structure or way of organizing your code so it is more readable / maintainable moving forward.

// This can be as a simple as consciously organizing your code so you have all variable declarations first, then your core function defintions
// then, all your listeners, just some system that makes it easier for future you to go back and build on.
...
// For example, you could make Game, and Question classes.
